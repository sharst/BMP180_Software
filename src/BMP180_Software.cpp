/*
  ...
  Adapted to use Software I2C by S. Harst
...
BMP180Soft::BMP180Soft()
// Base library type
{
}


char BMP180Soft::begin()
// Initialize library for subsequent pressure measurements
{
  double c3,c4,b1;
  
  // Start up the Arduino's "wire" (I2C) library:
  i2c_init();  // Returns 1 even if no sensor is attached.
...
char BMP180Soft::readInt(char address, int16_t &value)
...
char BMP180Soft::readUInt(char address, uint16_t &value)
...
char BMP180Soft::readBytes(unsigned char *values, char length)
...
// length: number of bytes to read
{
  char x;
  _error = 0;
  if (!i2c_start((BMP180_ADDR<<1) | I2C_WRITE)) _error=1;
  //Wire.beginTransmission(BMP180_ADDR);
  if (!i2c_write(values[0])) _error=1;
  //Wire.write(values[0]);
  i2c_stop();
  ...
  if (_error == 0)
  {
    i2c_start((BMP180_ADDR<<1) | I2C_READ);
    boolean stop = false;
    for(x=0;x<length;x++) {
      if (x==length-1) stop = true;
      values[x] = i2c_read(stop);
      //values[x] = Wire.read();
    }
    i2c_stop();
    return(1);
...
char BMP180Soft::writeBytes(unsigned char *values, char length)
...
// length: number of bytes to write
{
  ...
  //Wire.beginTransmission(BMP180_ADDR);
  for (x=0; x<length; x++) {
    if (!i2c_write(values[x])) _error=1;
  }
...
char BMP180Soft::startTemperature(void)
...
char BMP180Soft::getTemperature(double &T)
...
char BMP180Soft::startPressure(char oversampling)
...
char BMP180Soft::getPressure(double &P, double &T)
...
double BMP180Soft::sealevel(double P, double A)
...
double BMP180Soft::altitude(double P, double P0)
